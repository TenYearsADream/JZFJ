...
namespace BusinessLogic.SortingTask
{
    ...
    public class SortingLineTask : BusinessBase<SortingLineTask>
    {
        ...
        private static readonly PropertyInfo<int> PLCADDRESSProperty = RegisterProperty<int>(p => p.PLCADDRESS, "PLC虚出口号,手工线用作任务在小车中的地址");
        ...
        private void DataPortal_Fetch()
        {
            using (BypassPropertyChecks)
            {
                using (var cn = new MySqlConnection(AppUtil._LocalConnectionString))
                {
                    ...
                    {
                        cm.CommandText =
                            "SELECT * FROM T_SORTING_LINE_TASK t WHERE t.status = 0 and t.islock = 0 order by t.indexno limit 1";
                        //cm.Parameters.AddWithValue("@id", criteria.Value);
                        using (var dr = new SafeDataReader(cm.ExecuteReader()))
                        {
                            while (dr.Read())
