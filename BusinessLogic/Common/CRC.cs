...
namespace BusinessLogic.Common
{
    ...
    public class CRC
    {
        public static byte[] crc_16(byte[] Array, int Length)
        {
            ...
            for (i = 0; i < Length; i++)
            {
                crc16 ^= Array[i]; // CRC = BYTE xor CRC  
                for (j = 0; j < 8; j++)
                {
            ...
            {
                returnbytes[k] = Array[k];
            }
            byte[] bytes = intToBytes(crc16);
            returnbytes[Length] = bytes[0];
            returnbytes[Length + 1] = bytes[1];
            return returnbytes;
        }

        private static byte[] intToBytes(uint value)
        {
            byte[] src = new byte[2];
